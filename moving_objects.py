...
class ProcessImages:
    ...
    def load_model(self):
        ...
        config.display()
        # change me to /gpu:0 if CUDA acceleration is installed correctly
        DEVICE = '/cpu:0'
...
if __name__ == '__main__':
    ...
    while image_processor.block_list == []:
        image_processor.find_blocks()
    
    for obj_info in image_processor.block_list:
        if motion_controller.intercept(RIGHT_ARM, obj_info):
            print("got block")
        else:
            print("failed to get block")
            
    motion_controller.pick_place(RIGHT_ARM, [0.45, -0.205, 0.0], [0.0, 0.0, 0.0], PICK)
    motion_controller.pick_place(RIGHT_ARM, [0.35, -0.205, 0.0], [0.0, 0.0, 0.0], PLACE)
    motion_controller.pick_place(RIGHT_ARM, [0.45, -0.205, 0.0], [0.0, 0.0, 0.0], PICK)
    motion_controller.drop(RIGHT_ARM, [0.45, -0.205, 0.2])
