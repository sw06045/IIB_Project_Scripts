...
class ProcessImages:
    ...
    def load_model(self):
        ...
        DEVICE = '/cpu:0'
        MODEL_DIR = './src/foodly_felix/Mask_RCmapNN/Mask_RCNN/logs'
...
if __name__ == '__main__':
    ...
    for obj_info in image_processor.block_list:
        start, eef_pos, eef_dir = obj_info
        if eef_pos[2] < -0.2:
            print("bad")
            eef_pos[2] = -0.2
            
        if eef_pos[1] <= 0.0: 
            if not motion_controller.pick_place(RIGHT_ARM, eef_pos, eef_dir, PICK):
                continue
            motion_controller.drop(RIGHT_ARM, [0.45, -0.3, 0.0])
        else:
            if not motion_controller.pick_place(LEFT_ARM, eef_pos, eef_dir, PICK):
                continue
